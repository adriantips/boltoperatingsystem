...
hubbub_error emit_current_tag(hubbub_tokeniser *tokeniser)
{
	...
	if (token.type == HUBBUB_TOKEN_START_TAG) {
		/* Save start tag name for R?CDATA. NB: this is a fixed-length,
		 * length-tracked copy (NOT a C string), so it must copy exactly
		 * name.len bytes. BoltOS's strncpy is strlcpy-style -- it copies
		 * at most n-1 bytes and always NUL-terminates -- which would drop
		 * the last character of the tag name (e.g. "title"->"titl"), so
		 * the RCDATA/RAWTEXT close-tag match (</title>, </style>,
		 * </script>) would never fire and the element would swallow the
		 * rest of the document. memcpy copies the exact bytes upstream
		 * (with a conformant strncpy) intends. */
		if (token.data.tag.name.len <
			sizeof(tokeniser->context.last_start_tag_name)) {
			memcpy((char *) tokeniser->context.last_start_tag_name,
